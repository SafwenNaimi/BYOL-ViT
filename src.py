...
class ResNetFeatures(nn.Module):
    ...
    def __init__(self):
        ...
        pretrained_path =  os.path.join('Python Scripts/BYOL/experiments/self-supervised/byol_full_225.pth')
...
patch_size = 1
image_sz = 96
max_len = 150 # All sequences must be less than 1000 including class token
embed_dim = 768 #512
classes = 5
layers = 24
channels = 3
resnet_features_channels = 128
heads = 16
epochs = 100
...
optimizer = optim.SGD(model.parameters(), lr=lr, momentum=float(cfg.momentum), weight_decay=0.05, nesterov=True)
...
for epoch in range(epochs):
    ...
    running_loss, running_accuracy = train(model, dataloader_train, criterion, optimizer, scheduler, resnet_features)
    print(f"Epoch : {epoch+1} - acc: {running_accuracy:.4f} - loss : {running_loss:.4f}\n")
    train_accs.append(running_accuracy)

    test_loss, test_accuracy = evaluation(model, dataloader_val, criterion, resnet_features)
