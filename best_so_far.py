...
class ResNetFeatures(nn.Module):
    ...
    def __init__(self):
        ...
        pretrained_path =  os.path.join('Python Scripts/BYOL/experiments/self-supervised/new_byol.pth')
...
class ResNetFeatures(nn.Module):
    ...
    def __init__(self):
        super(ResNetFeatures, self).__init__()
        layers = list(resnet18(pretrained=False).children())[:8] #all layer expect last layer
...
logs=os.path.join('Python Scripts/VIT/zaama') #tboard
...
vall=val/epochs
writer.add_text('Average accuracy','acc {}'.format(vall))
writer.close()
