...
class ResNetFeatures(nn.Module):
    ...
    def __init__(self):
        ...
        pretrained_path =  os.path.join('Python Scripts/BYOL/experiments/self-supervised/byol_new_aug_500.pth')
...
torch.manual_seed(0)
config_path = r'Python Scripts/VIT/config_sl.yaml'
...
def train(model, dataloader, criterion, optimizer, writer, scheduler, resnet_features=None):
    '''
...
def evaluation(model, dataloader, writer, criterion, resnet_features=None):
    '''
...
#logs=os.path.join('Python Scripts/VIT/experiments/exp2') #tboard
writer = SummaryWriter(logs + '/vit_byol_500_layer2_p1_new_new' )
train_accs = []
test_accs = []
val=0
for epoch in range(epochs):
    
    running_loss, running_accuracy = train(model, dataloader_train, criterion, optimizer,writer, scheduler, resnet_features)
    print(f"Epoch : {epoch+1} - acc: {running_accuracy:.4f} - loss : {running_loss:.4f}\n")
    train_accs.append(running_accuracy)

    test_loss, test_accuracy = evaluation(model, dataloader_val,writer, criterion, resnet_features)
