...
class ResNetFeatures(nn.Module):
    ...
    def __init__(self):
        ...
        pretrained_path =  os.path.join('Python Scripts/BYOL/experiments/self-supervised/NEW_data_aug_more1000.pth')
...
stl10 = torchvision.datasets.STL10(root='Python Scripts/ViT/data', split="train", download=False, transform=train_transform)
dataloader_train = DataLoader(stl10, batch_size=16, num_workers=2, shuffle=False)
...
cifar10 = torchvision.datasets.STL10(root='Python Scripts/ViT/data',split="test", download=False, transform=test_transform)
...
dataloader_val = DataLoader(cifar10, batch_size=16,
...
def cls_train(train_loader, model, criterion, optimizer, epoch,resnet_features,writer):
    model.train()
    ...
    writer.add_scalar('train/Acc_epoch', avg_acc1, epoch)

from time import time
def cls_validate(val_loader, model, criterion,writer,resnet_features, epoch=None, time_begin=None):
    model.eval()
    loss_val, acc1_val = 0, 0
    n = 0
    with torch.no_grad():
        ...
        writer.add_scalar('validation/Acc_epoch', avg_acc1, epoch)
...
##optimizer = optim.Adam(model.parameters(),lr=float(lr), weight_decay=0.05)
...
def adjust_learning_rate(optimizer, epoch):
    ...
    lr *= 0.5 * (1. + math.cos(math.pi * (epoch - 5) / (epochs - 5)))
...
for epoch in range(epochs):
    ...
    cls_train(dataloader_train, model, criterion, optimizer, epoch,resnet_features,writer)
    acc1 = cls_validate(dataloader_val, model, criterion,writer,resnet_features, epoch=epoch, time_begin=time_begin)
