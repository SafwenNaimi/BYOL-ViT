...
class ResNetFeatures(nn.Module):
    ...
    def __init__(self):
        ...
        pretrained_path =  os.path.join('Python Scripts/BYOL/experiments/self-supervised//byol_cifar_600.pth')
        ...
        self.feature_extractor = torch.nn.Sequential(*list(encoder.children())[:5]) 
...
class ResNetFeatures(nn.Module):
    ...
    def __init__(self):
        super(ResNetFeatures, self).__init__()
        layers = list(resnet18(pretrained=True).children())[:5] #all layer expect last layer
...
def CIFAR10DataLoader(split, batch_size=8, num_workers=2, shuffle=True, size='32', normalize='standard'):
    ...
    if split == 'train':
        ...
        cifar10 = torchvision.datasets.CIFAR10(root='Python Scripts/ViT/data', train=True, download=True, transform=train_transform)
        ...
        cifar10 = torchvision.datasets.CIFAR10(root='Python Scripts/ViT/data', train=False, download=True, transform=test_transform)
        dataloader = DataLoader(cifar10, batch_size=batch_size, num_workers=num_workers, shuffle=shuffle)
...
writer = SummaryWriter(logs + '/vit_cifar_layer1_p77')
